#@func : for distributed training process
...
# __local_rank : 本地排名 | 
# __world_size : 进程组的总大小 | 
# __device : 设备类型 | 
# __initialized : 是否已初始化分布式进程组的标志 | 

def initialized():
    """
    @func : Used to determine whether the distributed environment has been initialized
...
def is_master():
    """
    @func : Whether it is the primary process (global ranking is 0)
...
def is_local_master():
    """
    @func : Whether the current process is the local host process (local rank is 0)
...
def new_group(ranks: List[int]):
    """
    @func : Creates a new process group, specifying which ranked processes belong to the group
...
def barrier():
    """
    @func : Make all processes wait here until all processes have reached this point. Often used to ensure synchronization
...
def broadcast(t: torch.Tensor, src_rank) -> None:
    """
    @func : Broadcast the tensor from the specified source process to all other processes.
...
def for_visualize(func):
    """
    @func : Decorator, which ensures that only the main process executes visuality-related functions.
...
def finalize():
    """
    @func : Destroy distributed process groups and clean up resources.
