...
def init_distributed_mode(local_out_path, only_sync_master=False, timeout=30):
    """
    @func : initialize the distributed training environment
...
class SyncPrint(object):
    """
    @func : output log files and terminals at the same time

    """

    def __init__(self, local_output_dir, sync_stdout=True):
        """
        @func : initializes the output file path and creates or appends log files
    ...
    def write(self, message):
        """
        @func : write logs to both files and terminals
    ...
    def flush(self):
        """
        @func : force data from the buffer to be written to a file
...
class DistLogger(object):
    """
    @func : control the log output level and determine whether to record logs based on the verbose parameter
...
class TensorboardLogger(object):
    """
    @func : use TensorBoard to record various metrics during training
...
class MetricLogger(object):
    """
    @func : track and record metrics in each iteration
    ...
    def log_every(self, 
                  ...
                  header=None):
        """
        @func : load the data obj for every training step
...
def create_npz_from_sample_folder(sample_folder: str):
    """
    @func : build a single .npz file from a folder of .png samples. Refer to DiT.
