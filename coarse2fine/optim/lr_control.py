...
def lr_wd_annealing(
    ...
    """
    @func : adaptive learning rate and weight decay
    """
    ...
    cur_wd = wd_end + (wd - wd_end) * (0.5 + 0.5 * math.cos(math.pi * pasd)) # like "cos" from wd to wde
    ...
    for param_group in optimizer.param_groups:
        ...
        # record max and min lr
        max_lr = max(max_lr, param_group['lr'])
        min_lr = min(min_lr, param_group['lr'])
        # update wd
        param_group['weight_decay'] = cur_wd * param_group.get('wd_sc', 1) # 'lr_sc' could be assigned
        # record max and min wd
...
def filter_params(model, nowd_keys=()) -> Tuple[
    ...
    """
    @func : filer the params for weight decay
    """
    ...
    for name, para in model.named_parameters():
        ...
        if para.ndim == 1 or name.endswith('bias') or any(k in name for k in nowd_keys):
            cur_wd_sc, group_name = 0., 'ND' # no need to decay
        else:
            cur_wd_sc, group_name = 1., 'D' # need to decay
        cur_lr_sc = 1.
