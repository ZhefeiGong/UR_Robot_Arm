...
class SharedAdaLin(nn.Linear):
    """
    :func: the shared conditional adaptive linear layer
    """
...
class VAR(nn.Module):
    def __init__(
        ...
        #### 7. observation encoder | borrow from `hybrid policy` of robomimic
        self.obs_encoder = obs_encoder                          # 🔥 🔥 🔥 
        self.obs_dim = obs_encoder.output_shape()[0]            # 🔥 🔥 🔥
        self.n_obs_steps = n_obs_steps                          # 137*n | 🔥 🔥 🔥 
        ...
        #### 2. observation embedding | condition on obs
        ...
        #### 5. attention mask used in training (for masking out the future) | it won't be used in inference, since kv cache is enabled
    ...
    def get_logits(self, 
        ...
        """
        :func: logical layer outputting the vocab's size probability
        """
        return self.head(self.head_nm(h.float(), cond_BD).float()).float() # from C to V | BLV | [batch_size, 70, 512]
    
    @torch.no_grad()
    def autoregressive_infer_cfg(
        self, 
        nobs: Optional[List[Union[int, torch.LongTensor]]],
        vae_proxy: VQVAE = None, # 🔥 🔥 🔥
    ) -> torch.Tensor: 
        """
        ...
        cur_L = 0 
        # f_hat = sos.new_zeros(B, self.Cvae, self.patch_nums[-1], self.patch_nums[-1]) # [B,Cvae,pn_last,pn_last]
        ...
        for si, pn in enumerate(self.patch_nums):   # si: i-th segment
            ...
            # idx_Bl = sample_with_top_k_top_p_(logits_BlV, rng=rng, top_k=100, top_p=0.96, num_samples=1)[:, :, 0] # [B,self.patch_nums[si]*1] | sample
            idx_Bl = logits_BlV.data.argmax(dim=-1) #  | maximize | 🔥 🔥 🔥
            # h_BChw = self.vae_quant_proxy[0].embedding(idx_Bl)   # [B, self.patch_nums[si]*1, Cvae]
            h_BChw = vae_proxy.idxBl_to_embeddings(idx_Bl)   # [B, self.patch_nums[si]*1, Cvae] | 🔥 🔥 🔥
            # h_BChw = h_BChw.transpose_(1, 2).reshape(B, self.Cvae, pn, pn) # [B, Cvae, pn, pn]
            h_BChw = h_BChw.transpose_(1, 2).reshape(B, self.Cvae, pn, 1) # [B, Cvae, pn, 1] | 🔥 🔥 🔥
            # f_hat, next_token_map = self.vae_quant_proxy[0].get_next_autoregressive_input(si, len(self.patch_nums), f_hat, h_BChw) # [B,Cvae,last_pn,last_pn] | [B,Cvae,next_pn,next_pn]
            ...
            if si != self.num_stages_minus_1:   # prepare for next stage
                ...
                # next_token_map = self.word_embed(next_token_map) + lvl_pos[:, cur_L:cur_L + self.patch_nums[si+1] ** 2] # [B,next_pn**2,embed_dim]
    ...
    def forward(self, 
        ...
        """
        @input:
        :param label_B: label_B has shape [batch_size, ] | [50, obs_dim]
        :param x_BLCv_wo_first_l: teacher forcing input [batch_size, total_num_patches_exept_first, self.Cvae] | [B, 679, 32]
        @return:
        ...
        x_BLV = self.get_logits(x_BLC.float(), cond_BD) # from C to V ｜ BLV | [B,680,4096] 
