...
from vqvae_sep import VQVAE, VectorQuantizer2
...
class VQVAETrainer(object):
    def __init__(
        ...
        # self.usage_names = ['x','y','z','rx','ry','rz','gripper']
    ...
    def train_step(
        ...
        with self.vae_opt.amp_ctx:
            ...
            # change BL7 into B1L7
            inp = self.vae_norm['action'].normalize(inp) # B|L|10
            inp = inp.view(inp.shape[0],1,inp.shape[1],inp.shape[2]).contiguous() # B|1|L|10
            
            # forward
            self.vae_wo_ddp.forward
            inp_slice = inp[:,:,:,self.act_dim_sep:self.act_dim_sep+1] # [B,1,16,1]
            ...
            # combine loss
            # loss_rec = self.w_l1*loss_rec_l1 + self.w_l2*loss_rec_l2
            # loss_vae = loss_rec + self.w_vq*loss_vq 
            loss_vae = self.w_l2 * loss_rec_l2 + self.w_vq * loss_vq # 🌪️
    ...
    def ema_update(self, g_it):
        """
        @func: 
        ema update in order to get a more stable version var_ema
        """
        ## init
