...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('ema_vocab_hit_V', torch.full((self.vocab_size,), fill_value=0.0)) # 💦 💦 💦
        self.record_hit = 0
        ...
        # ### ema codebook | 🔥 🔥 🔥
        # self.register_buffer('ema_count', torch.zeros(self.vocab_size))
        # self.ema_embeddings = nn.Parameter(torch.Tensor(self.vocab_size, self.Cvae))
        # self.ema_embeddings.data.normal_()
        # self.decay = 0.99
        # self.epsilon = 1e-5
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): 
                ...
                # if self.training:
                #     # ema count | Laplace smoothing of the cluster size
                #     self.ema_count = self.ema_count * self.decay + (1 - self.decay) * torch.sum(idx_N_one_hot, 0) # [vocab_size,]
                #     n = torch.sum(self.ema_count.data)
                #     self.ema_count = ((self.ema_count + self.epsilon) / (n + self.vocab_size * self.epsilon) * n)
                #     # ema embedding update
                #     dw = torch.matmul(idx_N_one_hot.float().t(), rest_NC) # [vocab_size,B*h*w] * [B*h*w, C] = [vocab_size, C]
                #     self.ema_embeddings = nn.Parameter(self.ema_embeddings * self.decay + (1 - self.decay) * dw)
                #     # update
                #     self.embedding.weight = nn.Parameter(self.ema_embeddings / self.ema_count.unsqueeze(1))
                
                # calculate vq loss
                vocab_hit_V.add_(hit_V) # hit times calculate
                mean_vq_loss += F.mse_loss(f_hat.data, f_BChw).mul_(self.beta)  # commit loss
                mean_vq_loss += F.mse_loss(f_hat, f_no_grad).mul_(self.gamma)   # vq loss
    ...
    # def idxBl_to_var_input(self, 
    #                        gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
    #     """
    #     @func: 
    #     only used in VAR training, for getting teacher-forcing input
    #     """
    #     next_scales = [] # 
    #     B = gt_ms_idx_Bl[0].shape[0] # batch_size
    #     C = self.Cvae # the channel of vqvae
    #     H = W = self.v_patch_nums[-1] # last patch's size
    #     SN = len(self.v_patch_nums)
    #     f_hat = gt_ms_idx_Bl[0].new_zeros(B, C, H, W, dtype=torch.float32) # [batch_size, Cvae, last_patch, last_patch] | [50,3,16,16]
    #     pn_next: int = self.v_patch_nums[0]
    #     for si in range(SN-1):
    #         # z_k from [batch_size, 1, Cvae] to [batch_size, Cvae, 1, 1] to(bicubic) [batch_size, Cvae, 16, 16]
    ...
    #         # \hat{f} from [batch_size, Cvae, 16, 16] to(area) [batch_size, Cvae, 2, 2] to [batch_size, 4, Cvae]
    ...
    #                                   SN: int, 
    #                                   f_hat: torch.Tensor, 
    #                                   h_BChw: torch.Tensor) -> Tuple[Optional[torch.Tensor], torch.Tensor]: # only used in VAR inference
    #     """
    #     @func: 
    #     only used in VAR inference, for getting next step's input
    #     """
