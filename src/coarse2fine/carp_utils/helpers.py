...
def sample_with_top_k_top_p_(logits_BlV: torch.Tensor, 
    ...
    @func: 
    @input: 
    logits_BlV has shape [B,self.patch_nums[si]*1,V]
    """
    B, l, V = logits_BlV.shape
    # the value which is lower than the topk-th = -torch.inf
    if top_k > 0:
        idx_to_remove = logits_BlV < logits_BlV.topk(top_k, largest=True, sorted=False, dim=-1)[0].amin(dim=-1, keepdim=True)
        logits_BlV.masked_fill_(idx_to_remove, -torch.inf)
    # the value which is except the first top_p probability = -torch.inf
    if top_p > 0:
